 ...
 *  -----------------------------------------------------------------------------------------------------------------*/
/**        \file  Lock_Door.c 
 *      \details  - This Project is about SmartDoor lockSystem you must enter the right pin for the Stepper motor 
...
static void Unlocked_Door_Routine(void){
	...
	SET_STEPPER_MOTOR(90,CLOCKWISE,12);
	_delay_ms(2800);
	SET_STEPPER_MOTOR(90,ANTICLOCKWISE,12);
...
static void Setting_New_Pin(void){
	...
	LCD_GoTo(1,5);
	Get_New_Password();
	AT24C32_WriteMultipleByte(AT24C32_Address,0X00,Current_pin,PIN_SIZE);
	LCD_Clear();
...
void SmartDoor_Initialize(void){
	...
	ServoMotor_Init();
	AT24C32_INIT();
	AT24C32_ReadMultipleByte(AT24C32_Address ,0X00,Current_pin,PIN_SIZE);
